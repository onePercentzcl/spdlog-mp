...
测试 100 个生产者（10 进程 × 10 线程）同时写入时的丢失率阈值。
...
**macOS 测试结果 (Apple M4 Pro)：**

| 延迟 | 实际写入 | 丢失率 | 吞吐量 |
|------|----------|--------|--------|
| 0 | 8,311 | 91.69% | 486K msg/s |
| 250μs | 41,827 | 58.17% | 129K msg/s |
| **300μs** | **100,000** | **0%** | **259K msg/s** |
| 500μs | 100,000 | 0% | 158K msg/s |

**Linux 测试结果 (RK3588, 5次平均)：**

| 延迟 | 实际写入 | 丢失率 | 吞吐量 |
|------|----------|--------|--------|
| 0 | 8,200-24,500 | 75-92% | 260-810K msg/s |
| 50μs | 18,600-53,000 | 47-81% | 93-231K msg/s |
| **100μs** | **36,800-100,000** | **0-63%** | **127-334K msg/s** |
| **200μs** | **100,000** | **0%** | **310-345K msg/s** |

**结论：**
- **macOS 零丢失阈值**：300μs（每条消息间隔）
- **Linux 零丢失阈值**：200μs（稳定），100μs（波动）
- Linux 性能优于 macOS 约 1.5 倍，得益于 futex 的高效实现
- 如需稳定零丢失，建议使用 **200μs** 作为安全阈值
...
cmake --build build
./build/example/example_mp4 --quick   # 快速测试
./build/example/example_mp4           # 完整测试
```
